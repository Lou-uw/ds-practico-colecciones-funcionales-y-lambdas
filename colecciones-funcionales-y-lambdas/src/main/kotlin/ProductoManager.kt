...
class ProductoManager {
    ...
    fun aplicarDescuento(
     ...
     return productos.map {it.precio - (it.precio * (descuentoPorcentaje/100.0))}
    }

    fun generarEtiquetas(productos: List<Producto>): List<String> {
        return productos.map {"${it.nombre} - $${it.precio} (${if(it.enStock)"Disponible" else "Agotado"})" }
    }

    // Parte B: Operaciones con Filter

    fun obtenerProductosEnStock(productos: List<Producto>): List<Producto> {
        return productos.filter {it.enStock == true }
    }
    ...
    fun aplicarDescuentoCategoria(
        ...
        return productos.filter {it.categoria == categoria }
            .map{it.precio - (it.precio * (descuentoPorcentaje/100.0)) }
    }
    ...
    fun generarReporteProductosCaros(
        ...
        return productos.filter {it.enStock == true && it.precio > precioMinimo}
